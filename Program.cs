    ...
    static void verMisDatos(Persona elemento){
        foreach(Persona persona in listaGeneral){
            if(elemento == persona){
                ...
                Console.WriteLine($"UUID: {persona.UUID} - Apellidos: {persona.Apellidos} - Nombres: {persona.Nombres} - Créditos: {((Alumno)persona).NumeroCreditos}");
                } else if(persona is Profesor){
                Console.WriteLine($"UUID: {persona.UUID} - Apellidos: {persona.Apellidos} - Nombres: {persona.Nombres} - Cargo: {((Profesor)persona).Cargo}");
                }
            }
        }
    }
